...
| **M6. MSL-ядро** (частично)        | Blocks, Electrical.Analog, Mechanics.Rotational                    | Библиотека в раскладке MSL; синтаксис MSL разбирается       |
...
2. **Длинный хвост семантики MSL (M6)** — частично закрыт: с инструментом поставляется стандартная библиотека в раскладке MSL, а синтаксис MSL (пакеты, точечные имена, псевдонимы типов, partial-классы, импорты, `within`, графические аннотации, `assert`, `noEvent`) разбирается. Настоящим MSL-файлам дополнительно нужны `replaceable`/`redeclare`, `inner`/`outer`, перечисления и условные компоненты.
