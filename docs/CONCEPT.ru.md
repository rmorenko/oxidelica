...
| **M9. Дискретный слой** (частично) | Дискретные переменные, `when`-уравнения, `pre`, `sample`           | Считаются цифровой регулятор и термостат с гистерезисом              |
...
3. **Дискретный слой (M9)** — событийная часть закрыта: переменные, меняющиеся только в событиях, `when`-уравнения с `elsewhen`, `pre`/`edge`/`change`/`initial()`, часы `sample(start, interval)` с точным попаданием шага в момент такта и итерация событий, когда одно событие вызывает цепочку клауз. Не хватает: секций `algorithm` в моделях (пока только в функциях) и `initial equation`, которому нужна отдельная система инициализации.
