...
Docker (ядро и GUI), а ядро дополнительно проходит проверку типов под
`x86_64-pc-windows-msvc`. Сборка GUI под Windows требует инструментов
MSVC — это остаётся за CI.

Linux'у нужны библиотеки, с которыми линкуется Bevy: на Debian и Ubuntu
`pkg-config libasound2-dev libudev-dev`. Сборку под Linux можно проверить
с макбука, не выходя из репозитория:
