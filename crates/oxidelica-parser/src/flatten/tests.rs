//! What the flattener is expected to do.
...
fn a_false_condition_removes_a_component_and_its_connections() {
    ...
    let kept =
        crate::parser::parse_model(&source.replace("measure = false", "measure = true")).unwrap();
...
fn a_binding_on_a_variable_is_an_equation() {
    let m = crate::parser::parse_model("model M Real x; Real y = 2 * x; equation x = time; end M;")
        .unwrap();
    assert_eq!(m.equations.len(), 2);
